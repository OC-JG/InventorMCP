...
def documented() -> dict[str, float]:
    found = {name: float(value) for name, value in ROW.findall(README.read_text(encoding="utf-8"))}
...
def fixtures() -> dict[str, float]:
    return {
        path.stem: json.loads(path.read_text(encoding="utf-8"))["volume_cm3"]
...
def test_the_comparison_table_quotes_every_example_and_agrees_with_the_files():
    ...
    """
    table = README.read_text(encoding="utf-8").split("## How close the simulator is")[1]
