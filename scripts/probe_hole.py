...
See ``scripts/apartment.py`` for why, and for what it looks like when the rule is
broken -- an ``AttributeError`` that reads like a missing property.
"""
...
def main(argv=None) -> int:
    ...
    def probe() -> list[str]:
        """Everything that touches a live COM object, on the apartment that owns it."""
        inner = raw(backend)
        document = inner._doc(context.doc_id)
        ...
        inner.build_sketch(context.doc_id, plan)
        sketch = inner._sketch(context.doc_id, "Probe0")
        ...
        for label, direction, through, height in TRIALS:
            ...
            inner.build_sketch(context.doc_id, plan)
            sketch = inner._sketch(context.doc_id, plan.name)
        ...
        inner.build_sketch(context.doc_id, plan)
        sketch = inner._sketch(context.doc_id, "Flip")
        centers = app.TransientObjects.CreateObjectCollection()
        ...
        try:
            placement = features.CreateSketchPlacementDefinition(centers)
            feature = features.AddDrilledByDistanceExtent(
                placement, "9 mm", "6 mm", inner._k("kPositiveExtentDirection"))
        except Exception as exc:
        ...
        if feature is not None:
            for name, route in (("HoleFeature.ExtentDirection", lambda: feature),
                ...
                try:
                    ...
                    target.ExtentDirection = inner._k("kNegativeExtentDirection")
                    com._recompute(document)
                    after = com._solid_volume(document)
                    moved = None if (before is None or after is None) else after - before
    ...
    worked = on_thread(backend, probe)
